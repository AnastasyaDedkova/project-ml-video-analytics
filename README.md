...
- отбирать кадры или короткие видеофрагменты для анализа;
...
Горизонт работы системы - текущий рабочий день клиники. Система должна выявлять подозрительные случаи не через несколько дней после приема, а с небольшой задержкой. Для MVP ориентиром является задержка выявления события до 1–3 минут.
...
Для решения задачи выбрана модель семейства YOLO. Такой выбор связан с тем, что YOLO подходит для детекции объектов в видеопотоке: модель достаточно быстрая, может использоваться в режиме, близком к реальному времени, и может дообучаться на собственных данных. В исходных материалах проекта YOLO также выбрана из-за скорости, точности и возможности дообучения на своем датасете. :contentReference[oaicite:0]{index=0}

### 2.2. Схема решения

В данном разделе отдельная блок-схема baseline/MVP не добавляется, чтобы не увеличивать количество диаграмм сверх требований задания. Логика решения уже частично отражена в BPMN-диаграмме процесса после внедрения, а техническая структура будет показана дальше на обязательной диаграмме архитектуры системы с распределенным хранением данных.
...
Первый этап - подготовка данных для обучения и проверки модели. Источником данных являются видеозаписи из стоматологических кабинетов. В исходном проекте видеозаписи собирались непосредственно в кабинетах, а для передачи потока использовался RTSP. Запись выполнялась через OpenCV, каждая видеозапись охватывала полный рабочий день с 8:00 до 20:00, что позволило собрать разные сценарии работы кабинета и разное освещение. :contentReference[oaicite:1]{index=1}
...
В исходных материалах описано, что после сбора данных выполнялось удаление похожих кадров, затем кадры размечались вручную. Датасет сохранялся в формате COCO, а в одном из вариантов было размечено 2000 кадров. :contentReference[oaicite:2]{index=2} Также в презентации по проекту указано, что датасет включал около 4 тыс. кадров из 11 кабинетов, а для подготовки использовались ffmpeg и Label Studio. :contentReference[oaicite:3]{index=3}
...
В исходных экспериментах сравнивались DETR, SSD, YOLOv10 и YOLOv11. Для выбора учитывались mAP@50, latency и вес модели. По результатам сравнения YOLOv11 показала лучший результат среди рассмотренных моделей: mAP@50 = 0.765, latency = 0.015 с, вес модели = 5.4 МБ. :contentReference[oaicite:4]{index=4}

Для MVP это важно, потому что модель должна быть не только точной, но и достаточно быстрой. Если модель будет слишком тяжелой, обработка видеопотока из нескольких кабинетов станет дорогой и медленной.

Формирование выборки выполняется через разделение данных на train, validation и test. При этом важно, чтобы в тестовую выборку попадали кадры, которые модель не видела при обучении. Желательно также проверять модель на разных кабинетах, чтобы оценить, насколько хорошо она переносится на новые условия.

#### Этап 3. Метрики качества

Для оценки качества модели используются метрики computer vision: precision, recall, F1-score, IoU, mAP@50 и latency.

Precision показывает, какая доля найденных моделью объектов действительно является правильной. Для бизнеса это связано с количеством ложных срабатываний. Если precision низкий, система будет создавать много лишних подозрительных кейсов.

Recall показывает, какую долю реальных объектов модель смогла найти. Для нашего кейса recall особенно важен, потому что пропущенный пациент может привести к пропущенному фактическому приему.

mAP@50 используется как основная метрика качества детекции. Она показывает среднюю точность модели при пороге IoU 0.5. В исходном ML-отчете цель была сформулирована как достижение качества выше 0.7 по mAP50 для задачи детекции пациентов в стоматологических кабинетах. :contentReference[oaicite:5]{index=5}

Latency показывает скорость обработки кадра. Эта метрика важна для production-системы, потому что система должна работать с видеопотоком и находить аномалии с небольшой задержкой.

Важно не использовать только accuracy. В этой задаче разные ошибки имеют разный бизнес-смысл. Если модель ошибочно находит пациента, это создает лишнюю проверку для службы безопасности. Если модель не находит пациента, хотя он есть, это может привести к пропуску подозрительного приема.
...
К ML-метрикам относятся precision, recall, F1-score, mAP@50 и latency. Precision важен, чтобы система не создавала слишком много ложных кейсов. Recall важен, чтобы система не пропускала фактические приемы пациента. Latency показывает, успевает ли модель обрабатывать кадры с нужной скоростью.
...
- модель уверенно различает состояния `Patient` и `Empty`;
- фактические интервалы приемов формируются без частого дробления одного приема на несколько частей;
- большая часть расхождений с расписанием попадает в кейсы аномалий;
- служба безопасности не перегружается ложными уведомлениями;
- задержка создания кейса остается в пределах примерно 1–3 минут.
...
На этапе пилота также нужно определить правила хранения данных. Не обязательно хранить все видео полностью. Для проверки кейсов достаточно сохранять выбранные кадры, короткие фрагменты и метаданные: камера, кабинет, время, результат детекции, confidence и ссылка на кейс.
...
Перед описанием взаимодействий покажем структуру данных, которая используется в системе.

![ER-диаграмма структуры данных](diagrams/er-model.png)

Рисунок 3 - ER-диаграмма структуры данных системы
...
Рисунок 4 - Архитектура ML-системы и распределенного хранения данных

На архитектурной диаграмме видно, что система не хранит все данные в одном месте. Видеофрагменты и кадры находятся в MinIO / S3, операционные сущности - в PostgreSQL, события между сервисами передаются через Message Broker, а агрегированные данные уходят в BI / Superset. Такое распределение выбрано потому, что видеоданные имеют большой объем, а бизнес-данные требуют структурированного хранения и целостности.
...
Для production-версии система должна работать в режиме, близком к near real-time. Полная обработка каждого кадра в реальном времени не требуется, потому что бизнес-задача - не мгновенно управлять медицинским процессом, а своевременно находить подозрительные приемы.

Ориентировочные требования:

- доступность системы в рабочее время клиники - около 99,5%;
- задержка выявления события - до 1–3 минут;
...
- события детекции и аномалий не должны теряться;
...
Рисунок 5 - UML-диаграмма компонентов системы
