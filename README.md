...
- формировать результаты проверки за следующий рабочий день после обработки видеозаписей;
...
На диаграмме видно, что основная нагрузка находится на сотруднике, который вручную ищет подозрительные случаи. При росте числа кабинетов такой процесс становится неудобным: проверка занимает много времени.
...
В качестве частоты обработки можно использовать один кадр в две секунды. Это снижает вычислительную нагрузку и объем данных, но остается достаточным для определения факта нахождения пациента в кресле.
...
Для решения задачи выбрана модель семейства YOLO. Такой выбор связан с тем, что YOLO подходит для детекции объектов в видеопотоке: модель достаточно быстрая, может использоваться в режиме, близком к реальному времени, и может дообучаться на собственных данных.

### 2.2. Пайплайн решения

Пайплайн решения строится вокруг обработки видеоданных за прошедший рабочий день. В MVP система не работает как real-time мониторинг, а выполняет постобработку видеозаписей: получает записи с камер, извлекает из них кадры, применяет модель детекции и затем формирует фактические интервалы приемов.

Первый этап пайплайна — получение видеоданных из стоматологических кабинетов. Камеры записывают происходящее в кабинете, после чего видеозаписи используются для дальнейшей обработки. Для MVP достаточно обрабатывать записи постфактум, например на следующий день после рабочей смены.

Второй этап — нарезка видео на кадры. Для снижения вычислительной нагрузки анализируется не каждый кадр видеопотока, а кадры с заданной частотой. Например, можно использовать выборку около одного кадра в две секунды. Такой частоты достаточно, чтобы определить факт нахождения пациента в кресле и не обрабатывать лишние данные.

Третий этап — детекция пациента на кадрах. Для этого используется модель семейства YOLO. На вход модель получает кадр из стоматологического кабинета, а на выходе возвращает результат детекции: найден ли пациент, где он находится на изображении и с какой уверенностью модель сделала предсказание.

Четвертый этап — объединение результатов детекции во временной интервал. Отдельный кадр сам по себе не является приемом, поэтому последовательность кадров анализируется во времени. Если пациент обнаруживается на серии кадров, система фиксирует начало фактического приема. Если пациент перестает обнаруживаться в течение заданного времени, система фиксирует окончание приема. Короткие пропуски можно сглаживать, чтобы один прием не дробился на несколько частей из-за ошибки модели или перекрытия пациента врачом.

Пятый этап — сопоставление фактического интервала с расписанием. Система берет сформированный фактический прием и сравнивает его с данными из МИС: кабинетом, врачом, плановым временем начала и окончания приема. Если подходящая запись найдена, прием считается нормальным. Если записи нет или время сильно отличается от планового, создается кейс аномалии.

Шестой этап — подготовка результата для проверки. Система сохраняет фактические интервалы, результаты сопоставления, найденные аномалии и ссылки на подтверждающие кадры или видеофрагменты. Сотрудник службы безопасности уже не просматривает весь видеопоток подряд, а работает с подготовленными кейсами.

Таким образом, пайплайн решения выглядит так:

1. Получение видеозаписей из стоматологических кабинетов.
2. Нарезка видео на кадры с заданной частотой.
3. Детекция пациента на кадрах с помощью YOLO-модели.
4. Объединение результатов детекции во временной интервал приема.
5. Получение расписания из МИС.
6. Сопоставление фактического приема с расписанием.
7. Создание кейса аномалии при расхождении.
8. Передача кейса сотруднику службы безопасности для проверки.
9. Сохранение агрегированных данных для отчетности.

Главное отличие от ручного процесса состоит в том, что человек подключается не на этапе поиска подозрительного приема во всем видео, а на этапе проверки уже найденного системой кейса.
...
Финальный выбор - микросервисная архитектура, потому что система работает с разнородной нагрузкой: видео, ML, расписание, уведомления и отчеты. Если объединить все в один сервис, будет сложнее масштабировать обработку видео и независимо обновлять модель.
...
- срок формирования результатов - до конца следующего рабочего дня после анализируемой смены;
- частота анализа - примерно 1 кадр в 2 секунды на камеру;
- масштабирование - от нескольких кабинетов на пилоте до 100 камер при развитии;
...
Работа не сразу шла ровно, потому что в проекте смешались две большие части: ML System Design и архитектура информационной системы. Насте было сложнее разбираться в backend-части, микросервисах, интеграциях и инфраструктуре. Артему, наоборот, было сложнее погружаться в ML-часть: датасеты, метрики качества модели, разметку и особенности инференса. Но в итоге мы смогли разделить зоны ответственности и найти общий язык: я больше занималась ML, данными и методологией, а Артем - архитектурой, сервисами и backend-логикой.
